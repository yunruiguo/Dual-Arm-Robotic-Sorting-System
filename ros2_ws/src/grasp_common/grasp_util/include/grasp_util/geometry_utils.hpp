...
#ifndef GRASP_UTIL__GEOMETRY_UTILS_HPP_
#define GRASP_UTIL__GEOMETRY_UTILS_HPP_
...
// Find the nearest path-point index
inline size_t find_closest_pose_idx(
  ...
  for (size_t idx = 0; idx < path_poses.size(); ++idx) {
    double dist = euclidean_distance(current_pose, path_poses[idx], is_3d);
    if (dist < min_dist) {
      min_dist = dist;
...
// Compute trajectory length from the nearest point to the end
inline double calculate_remaining_path_length(
    const geometry_msgs::msg::Pose& current_pose,
    const moveit_msgs::msg::RobotTrajectory& trajectory,
    moveit::planning_interface::MoveGroupInterfacePtr& move_group) {
  // Extract pose list from trajectory points
  std::vector<geometry_msgs::msg::Pose> path_poses;

  // Iterate over `trajectory`,convert each trajectory point to an end-effector pose(Cartesian position)
  for (const auto& point : trajectory.joint_trajectory.points) {
    // Set `MoveGroupInterface` joint positions in
    move_group->setJointValueTarget(point.positions);

    // Get end-effector pose
    geometry_msgs::msg::PoseStamped end_effector_pose_stamped =
        move_group->getCurrentPose();
    path_poses.push_back(end_effector_pose_stamped.pose);
  }

  // Find the nearest point index for the current pose in the path
  size_t closest_idx = find_closest_pose_idx(current_pose, path_poses);

  // Accumulate distance from closest_idx to the path end
  double remaining_path_length = 0.0;
  for (size_t i = closest_idx; i < path_poses.size() - 1; ++i) {
    remaining_path_length +=
        euclidean_distance(path_poses[i], path_poses[i + 1]);
  }

  return remaining_path_length;
}

}  // namespace geometry_utils
}  // namespace grasp_util

#endif  // GRASP_UTIL__GEOMETRY_UTILS_HPP_
