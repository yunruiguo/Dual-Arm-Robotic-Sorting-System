...
#ifdef HAS_SIGACTION
inline struct sigaction set_sigaction(int signal_value,
                                      const struct sigaction &action)
#else
inline signal_handler_t set_signal_handler(int signal_value,
...
#endif
{
  ...
  if (ret == -1)
#else
  signal_handler_t old_signal_handler =
      std::signal(signal_value, signal_handler);
 ...
 * @return
 */
inline bool ok() { return g_signal_status == 0; }
/**
 * @brief shutdown
 */
inline void shutdown() { trigger_interrupt_guard_condition(SIGINT); }
