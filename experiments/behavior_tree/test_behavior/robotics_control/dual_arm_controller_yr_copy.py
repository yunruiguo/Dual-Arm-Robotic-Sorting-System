...
LastEditTime: 2025-04-02 19:50:00
...
class GraspTaskManager(Node):
    def __init__(self):
        ...
        self.max_retry_grasp_times = 5 # 
        self.current_retry_grasp_times = 0 #
        self.current_retry_observe_index = 0 #
    ...
    # yr   
    def single_arm_grasp_sequence(self, res_obj, category_list, flag, left_place_pos, right_place_pos):
        index = 0
        for info in res_obj: # 
    ...
    def generate_dual_arm_sequence(self, left_dularm_list, right_dularm_list, fixed_position,left_place_pos,right_place_pos):
        ...
        while idx_count < max_steps:
            category_obj, pose_left = left_dularm_list[idx_count]
            category = category_obj.split('-')[0]
            try:
                categoty_index = category_list.index(category)
            except ValueError:
                ...
                print(" no such category, che")
                return
            index_left = categoty_index % 2

            pose_left_copy = pose_left.copy()
            pose_left_copy[2] += 50  # 5cm

            if idx_count == 0:
                self.add_action_to_sequence('moveTo', 0, position=pose_left_copy)
            else:
                self.add_action_to_sequence(
                ("moveTo", "moveTo"),
                (0, 1),
                kwargs1={'position': pose_left_copy},  # 5cm
                kwargs2={'position': right_place_pos[index_right]}  # 
            )
            self.add_action_to_sequence('moveTo', 0, position=pose_left)  # 
            ...
            pose_left_up = pose_left.copy()
            pose_left_up[2] += 150  # z15cm
        
            self.add_action_to_sequence(
                ("moveTo", "moveTo"),
                (0, 1),
                kwargs1={'position': pose_left_up},  # 15cm
                kwargs2={'position': self.right_retry_pose[self.current_retry_observe_index]}  # 
            )
            
            category_obj, pose_right = right_dularm_list[idx_count]
            category = category_obj.split('-')[0]
            try:
                categoty_index = category_list.index(category)
            except ValueError:
                ...
                print(" no such category, che")
                return
            index_right = categoty_index % 2
            pose_right_copy = pose_right.copy()
            pose_right_copy[2] += 50  # 5cm
            
            self.add_action_to_sequence(
                ("moveTo", "moveTo"),
                (0, 1),
                kwargs1={'position': left_place_pos[index_left]},  # 
                kwargs2={'position': pose_right_copy}  # 5cm
            )

            self.add_action_to_sequence('moveTo', 1, position=pose_right)  # 
            self.add_action_to_sequence(
            ...
            pose_right_up = pose_right.copy()
            pose_right_up[2] += 150  # z15cm

            self.add_action_to_sequence(
                ("moveTo", "moveTo"),
                (0, 1),
                kwargs1={'position': self.left_retry_pose[self.current_retry_observe_index]},  # 
                kwargs2={'position': pose_right_up}  # 15cm
            )
            idx_count += 1
        ...
        if max_steps == 0:
            ...
            self.single_arm_grasp_sequence(self, res_obj, category_list, flag, left_place_pos, right_place_pos)

            # ,Returns
            if flag == 1:
                self.add_action_to_sequence('moveTo', flag, position=self.right_retry_pose[self.current_retry_observe_index])  # 
            else:
                self.add_action_to_sequence('moveTo', flag, position=self.left_retry_pose[self.current_retry_observe_index])  # 

        # ,   
        else: 
            if len(left_dularm_list) == len(right_dularm_list):  # ,
                self.add_action_to_sequence('moveTo', 1, position= right_place_pos[index_right])  # 
                self.add_action_to_sequence('openGripper', 1, force=50)
                
                self.add_action_to_sequence('moveTo', 1, position=self.right_retry_pose[self.current_retry_observe_index])
                return
            # res_obj  flag
            (res_obj, flag) = (left_dularm_list[max_steps:], 0) if (len(left_dularm_list) > len(right_dularm_list)) else (right_dularm_list[max_steps:], 1)
    ...
    def grasp_desktop_obj(self):
        if self.current_retry_grasp_times == 0:
            if not self.arm_controllers[0].feedback_received or not self.arm_controllers[1].feedback_received :
                self.get_logger().info(",")
                #return None
            self.initialized = True
        ...
        self.add_action_to_sequence('openGripper', 1,force=50)
        self.get_logger().info(",{}".format(self.current_retry_grasp_times))
        ...
        self.current_retry_grasp_times += 1
        self.current_retry_observe_index += 1
        self.current_retry_observe_index %= 3 # 
    ...
    def manage_task_execution(self):
        ...
        if self.action_queue.is_empty():
            self.is_executing = False
            # yr
            if self.current_retry_grasp_times < self.max_retry_grasp_times and self.is_grasp_desktop_obj_task:
