...
class GraspTaskManager(Node):
    def __init__(self):
        ...
        self.max_retry_grasp_index = 3 # 
        self.current_retry_grasp_index = 0 #
        self.is_grasp_desktop_obj_task = False #
        self.retry_pose = [[-467.147, -279.828, 45.00, 170.818*np.pi/180, 2.082*np.pi/180, -80.875*np.pi/180], # [-294.690, -393.499, 113.286, 160.732, 22.264, -66.895]
    ...
    def _process_target_mode(self, depth_to_base, transformation_matrix, rpy_xyz=None, mode="target", knife=True, 
        ...
        if mode == "6D":
             # z1cm
            current_translation = current_translation + np.array([0,0,28]) @ object_to_base_translation[:3, :3]
            ...
            if is_transform_to_other_arm:
                ...
                # # z1cm
                current_translation_in_other_arm = current_translation_in_other_arm + np.array([0,0,28]) @ object_to_other_arm_base_translation[:3, :3]
                ...
                filename = f"arm_rotation_record_{timestamp}.txt"
                log_filename = os.path.join("outputs/dual_arm_control_log/right_arm_rotation_record", filename)
                with open(log_filename, "w") as log_file:
                    action = f",theta{euler_angles_in_other_arm}"
                    log_file.write(action + "\n")
                    theta_add = (euler_angles_in_other_arm[2]*180/np.pi + 90)*np.pi/180
                    euler_angles_in_other_arm_add = np.array([179*np.pi/180, 0*np.pi/180, theta_add])
                    action = f"90,theta{euler_angles_in_other_arm_add}"
                    log_file.write(action + "\n")
    ...
    def _dularm_coordinate_transformation(self, depth_to_base, transformation_matrix, arm_index=1):
        """
        ,.
    ...
    def calculate_6d_position(self):
        ...
        with self.lock:
            ...
            if all(results):
                for idx, result in enumerate(results):
                    if idx == 0: # category_classify
                        ...
                        if result:
                            ...
                            pred_grasp_6d_all = result
                        else:
                            print(" get 6d_grasp results failed !!!")
                ...
                return
            # obj_can_grasp = {'category1-obj':(score, translation, rotation, category), 
            #                'category2-obj':(score, translation, rotation, category)}
            obj_can_grasp = select_best_grasp(pred_grasp_6d_all, processed_masks_dict, self.left_camera_intrinsics)
            ...
            left_arm_grasp_obj_items = ['Fruit','Snack','Beverage']
            right_arm_grasp_obj_items = ['Tool','Other']
            left_dularm_list = []
            right_dularm_list = []
            all_pose_in_left_arm = []
            all_pose_in_right_arm = []

            for category_obj, value in obj_can_grasp.items():
                score, translation, rotation, category = value
                xyz_rpy, xyz_rpy_in_other_arm = self.get_targetpose(translation, knife=False, rot_6d=rotation, 
                                                                    is_transform_to_other_arm=True, mode="6D", arm_index=0)# 0
                # 
                if category in left_arm_grasp_obj_items:
                    left_dularm_list.append((category_obj, xyz_rpy))
                elif category in right_arm_grasp_obj_items:
                    right_dularm_list.append((category_obj, xyz_rpy_in_other_arm))
                all_pose_in_left_arm.append((category_obj, xyz_rpy))
                all_pose_in_right_arm.append((category_obj, xyz_rpy_in_other_arm))

            fixed_position = {'left_observe_pos':[-392.240, -416.552, 12.916, 166.193*np.pi/180, 32.284*np.pi/180, -83.703*np.pi/180], # 
                               'right_observe_pos':[325.074, -487.712, -51.765, 153.587*np.pi/180, 9.913*np.pi/180, 168.613*np.pi/180 ],  # 
                               'left_place_pos':[-332.175, -764.108, -230.085, 173.738*np.pi/180, 6.420*np.pi/180, 6.150*np.pi/180], # 
                               'right_place_pos':[264.855, -759.292, -234.312, 179.998*np.pi/180, 0*np.pi/180, 90*np.pi/180]}# 
            
            # self.generate_dual_arm_sequence(left_dularm_list, right_dularm_list, fixed_position)
    ...
    def grasp_desktop_obj(self):
        ...
        self.add_action_to_sequence('moveTo', 0, position= self.retry_pose[self.current_retry_grasp_index])
        self.add_action_to_sequence('wait', 0, params=3, decision_logic=self.calculate_6d_position)
