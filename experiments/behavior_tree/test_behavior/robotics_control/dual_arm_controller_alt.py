'''
FilePath: dual_arm_controller_alt.py
Author: Yunrui Guo
Date: 2024-12-26 11:59:14
LastEditors: Please set LastEditors
LastEditTime: 2025-04-11 16:00:22
...
class GraspTaskManager(Node):
    def __init__(self):
        ...
        self.pose_buffer = deque(maxlen=8)  #  8 
        ...
        self.subscription = self.create_subscription(
            Float32MultiArray,
            '/hggd_grasp_detection_results',
            self.hggd_grasp_pose_callback,
            10
        )
        # 1️⃣ 
        self.action_map = {
        ...
        self.timer = self.create_timer(1, self.manage_task_execution)  # ****
         #  ROS 2 , 0.1s 
        self.wait_timer = self.create_timer(0.2, self.wait_callback)
    ...
    def load_camera_poses(self):
        """"""
        try:
            self.right_camera_pose = np.genfromtxt(str(REAL_CONFIG_DIR / 'alt_right_camera_pose.txt'), delimiter=' ')
            self.left_camera_pose = np.genfromtxt(str(REAL_CONFIG_DIR / 'alt_left_camera_pose.txt'), delimiter=' ')
    ...
    def hggd_grasp_pose_callback(self,msg):
        translations = []
        rotations = []
        scores = []

        # Extract and reshape data for each grasp
        for i in range(int(len(msg.data)/13)):
           
            grasp_data = msg.data[int(13*i):int(13*(i+1))]
            
            translation = grasp_data[:3]
            rotation = grasp_data[3:12]
            score = grasp_data[-1]
            
            translations.append(translation)
            rotations.append(np.array(rotation).reshape(3, 3))
            scores.append(score)
        ...
        # Assign the corresponding translation and rotation to self.xyz and self.rot
        self.hggd_grasp_translation = translations[max_score_index] # *1000mm
        self.hggd_grasp_rot = rotations[max_score_index]
        self.get_logger().info("hggd")
    ...
    def subscription_box_callback(self,grasppose: Float64MultiArray):
        ...
        if len(grasppose.data) < 27:
            ...
            return
        data=np.array(grasppose.data[0:27])/1000
        # ,
        self.grasp_points = {
            "topleft": list(data[0:3]),
            "topcenter": list(data[3:6]),
            "topright": list(data[6:9]),
            "centerleft": list(data[9:12]),
            "center": list(data[12:15]),
            "centerright": list(data[15:18]),
            "downleft": list(data[18:21]),
            "downcenter": list(data[21:24]),
            "downright": list(data[24:27])
        }
    ...
    def subscription_grasp_callback(self,grasp_pose):
        ...
        # ,
        self.grasp_info = {
            "x": grasp_pose.data[0]/1000,
            "y": grasp_pose.data[1]/1000,
            "z": grasp_pose.data[2]/1000,
            "angle": grasp_pose.data[3],
            "width": grasp_pose.data[4],
            "qaulity": grasp_pose.data[5],
            "grasp_depth": grasp_pose.data[6]/1000,
        }
        # 
        self.grasp_pose_update_needed = True
    ...
    def _process_target_mode(self, depth_to_base, transformation_matrix, rpy_xyz=None, mode="target", knife=True):
        ...
        if knife:
            ...
            knife_offset = np.array([0, 0.008, 0.08423])
    ...
    def openbox_pose(self):
        ...
        # 
        RPY_xyz = self.compute_pose(grasp_points["centerleft"], grasp_points["topleft"], grasp_points["centerright"],0)
        ...
        #  RPY 
        rpy_adjusted = np.array([-180*np.pi/180, -10*np.pi/180, centerleft_point1[5]])
        ...
        rpy_adjusted2 = np.array([-180*np.pi/180, -10*np.pi/180, centerleft_point1[5]])
        ...
        # 
        posesey = [-0.108615377171947, -0.7052641426520841, 0.509745836, -3.1416, 0.0, -1.5708]
        move_sequence = [
    ...
    def start_next_task(self):
        ...
        if self.current_task is None or self.action_queue.is_empty():  #  action_queue 
            ...
            if self.current_task == "grasp_knife":
                ...
                self.test()
            elif self.current_task == "test_grasp_knife":
                self.test_grasp_knife()
            else:
    ...
    def add_tasks(self):
        ...
        # self.task_queue =["test"]
        # self.task_queue =["test_grasp_knife"]
    ...
    def grasp4D_place(self):
        if self.grasp_info["x"]!=0.0:
            ...
            grasp_pose[2]=grasp_pose[2]-grasp_points["grasp_depth"]-0.008
            grasp_pose1=grasp_pose.copy()
            grasp_pose1[2]=grasp_pose1[2]+0.051
    ...
    def test_grasp_knife(self):
        ...
        self.add_action_to_sequence(
            ...
            (1, None),
            decision_logic=(None, None),
           # kwargs2={'tcp': [-0.263265,-1.0820139,0.484060,-3.141593,0.0,-1.570546]},
            # kwargs1={'tcp': [-0.17075477,-0.3519292,0.259745836,-3.141593,0.0,-1.570546]},#

            kwargs1={'tcp': [-0.17075477,-0.2019292,0.239745836,-3.141593,0.0,-1.570546]},#
        )
        # self.add_action_to_sequence(
        #     ("jointTo", None), 
        #     (1, None),
        ...
        #     kwargs1={'joint': [-2.0304393,2.116414,-0.3675561,1.13667976,0.11677429,0.9068463,1.278333]},#
        # )
    ...
    def test(self):
        ...
        #    # kwargs2={'tcp': [-0.263265,-1.0820139,0.484060,-3.141593,0.0,-1.570546]},
        #     # kwargs1={'tcp': [-0.17075477,-0.3519292,0.259745836,-3.141593,0.0,-1.570546]},#
        #     kwargs1={'tcp': [-0.108615377171947, -0.7052641426520841, 0.559745836, -180*np.pi/180, 10*np.pi/180,-90*np.pi/180]},#
        # )
        self.add_action_to_sequence(
            ("jointTo", None), 
            (1, None),
           ...
           # kwargs2={'tcp': [-0.263265,-1.0820139,0.484060,-3.141593,0.0,-1.570546]},
            # kwargs1={'tcp': [-0.17075477,-0.3519292,0.259745836,-3.141593,0.0,-1.570546]},#
            # kwargs1={'joint': [-0.01541171,0.1729083,0.1965173,0.6346606,0.568603,0.557122,1.278333]}
            kwargs1={'joint': [1.93346297740,0.253562241,2.290688753,0.7729346,-0.19529493,1.0182995,0.79441]},#
        )
        # self.add_action_to_sequence(
        #     ("moveTo", None), 
        #     (1, None),
        ...
        #    # kwargs2={'tcp': [-0.263265,-1.0820139,0.484060,-3.141593,0.0,-1.570546]},
        #     # kwargs1={'tcp': [-0.17075477,-0.3519292,0.259745836,-3.141593,0.0,-1.570546]},#
        #     kwargs1={'tcp': [-0.16045068255070633, -0.2476370687441082, 0.1108402645634465, -3.1416, 0.0, -1.5708]},#
        # )
        # -0.1668850925558002, -0.2749352984357164, 0.136103213992378
        # self.add_action_to_sequence( 
        #     ("moveTo", "moveTo"), 
        #     (0, 1),
        #     decision_logic=(None, None),
        #     kwargs1={'tcp': [0.30344033,-0.754681,0.485787,-3.141593,-0.0,-1.570546]},
        #     kwargs2={'tcp': [-0.263265,-1.0820139,0.484060,-3.141593,0.0,-1.570546]},
        #     # kwargs2={'tcp': [0.1240752414,-0.67711991,0.0620679,1.5046902,-0.02608652,-0.0999747]},#
        # )
        time.sleep(1)
    ...
    def grasp6D_place(self):
        while (len(self.hggd_grasp_translation)==0) or (len(self.hggd_grasp_rot) == 0):
            self.get_logger().info("Waiting for hggd grasp xyz and rot_matrix...")
    
        best_translation = self.hggd_grasp_translation
        best_rotation = self.hggd_grasp_rot
        hggd_rpy = R.from_matrix(best_rotation).as_euler('xyz', degrees=False)


        xyz_rpy = self.get_targetpose(best_translation, rpy_xyz =hggd_rpy, knife=False, mode="6D", arm_index=0)# 0
        
        # self.add_action_to_sequence('moveTo', 0, position= xyz_rpy)
      
        self.get_logger().info(f"############ hggd_6d: {xyz_rpy}")
    ...
    def place_object(self):
        ...
        self.add_action_to_sequence(
            ...
            (1, None),
            decision_logic=(None, None),
            kwargs1={'tcp': [-0.1617208489, -0.252149668-0.03, 0.1208403, -3.1416, 0.0, -1.5708]},
        )
        self.add_action_to_sequence(
            ("moveTo", None), 
            (1, None),
            decision_logic=(None, None),
            kwargs1={'tcp': [-0.1617208489, -0.252149668+0.001, 0.1208403, -3.1416, 0.0, -1.5708]},
        )
        self.add_action_to_sequence(
            ("closeGripper", None), 
            (1, None),
        ...
        self.add_action_to_sequence(
           ...
           # kwargs2={'tcp': [-0.263265,-1.0820139,0.484060,-3.141593,0.0,-1.570546]},
            ...
            kwargs1={'tcp': [-0.16334091687139177, -0.25242146625177997,0.239745836,-3.141593,0.0,-1.570546]},#
        )
        self.add_action_to_sequence(
            ("moveTo", None), 
            (1, None),
    ...
    def grasp_object(self):
        ...
        self.add_action_to_sequence(
            (None,"wait" ), 
            (None,0 ),
            decision_logic= (None,self.grasp4D_place),
            kwargs2={'params': 1},
        )
        # self.add_action_to_sequence('moveTo', 0, position=[-321.146,-506.292,36.00,3.1416,0.0,0.0])
    ...
    def open_box(self):
        ...
        self.add_action_to_sequence(
            ...
            (1, None),
            decision_logic=(None, None),
            kwargs1={'tcp': [-0.108615377171947, -0.7052641426520841, 0.559745836, -180*np.pi/180, 0*np.pi/180, -90*np.pi/180,]},
        )
        # self.add_action_to_sequence(
        #     ("jointTo", None), 
        #     (1, None),
        ...
        #     # kwargs1={'tcp': [-0.17075477,-0.3519292,0.259745836,-3.141593,0.0,-1.570546]},#
        #     # kwargs1={'joint': [-0.01541171,0.1729083,0.1965173,0.6346606,0.568603,0.557122,1.278333]}
        #     kwargs1={'joint': [2.651054382,-0.301714867,3.042267560,0.61201041,1.51267290,0.801840543,-0.25140509]},#
        # )
    ...
    def calculate_target_pose(self, feedback_data=None):
        ...
        graspose[:3] += np.array([0.0,0.03,-0.055])
        graspose = [graspose[0], graspose[1], graspose[2], -3.1416, 0.0, -1.5708]
        ...
        self.add_action_to_sequence(
           ...
           # kwargs2={'tcp': [-0.263265,-1.0820139,0.484060,-3.141593,0.0,-1.570546]},
            # kwargs1={'tcp': [-0.17075477,-0.3519292,0.259745836,-3.141593,0.0,-1.570546]},#
            kwargs1={'tcp': [graspose[0],graspose[1],0.239745836,-3.1416,0.0,-1.5808]},#
        )
        self.add_action_to_sequence(
            ("moveTo", None), 
            (1, None),
