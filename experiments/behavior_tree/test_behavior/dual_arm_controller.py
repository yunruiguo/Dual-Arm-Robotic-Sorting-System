'''
FilePath: dual_arm_controller.py
Author: Yunrui Guo
Date: 2024-12-26 11:59:14
LastEditors: Please set LastEditors
LastEditTime: 2025-03-27 16:54:30

Descripttion: 
'''
import rclpy
from pathlib import Path
REAL_CONFIG_DIR = Path(__file__).resolve().parent / "real"                                  # ROS2 Python library
import numpy as np    
import time                        # library
import math                                   # library
...
class GraspTaskManager(Node):
    ...
    def moveTo(self,index, tcp,decision_logic=None):
        ...
        task = self.create_message("moveTo", index, decision_logic, tcp=tcp)  
        if task is None:
            self.get_logger().error("❌ moveTodecision_logic ")
            return
        self.task_action_queue.add_message(task, None)  # ✅ 
        self.execute_arm_control(index, arm_command)
    ...
    def CheckArmFinished(self, task):
        is_executing = True
        action_name = task.action_func.__name__
        controller = self.arm_controllers[task.index]  # 
        # 1️⃣ 
        ...
        if task.decision_logic:
            logic = task.decision_logic
            if logic:
                self.get_logger().info(f"🧠 {action_name} ,: {logic.__name__}")
                logic()
            task.decision_logic = None  # ,
    ...
    def checkActionFinished(self):
        ...
        # ret = True
        # if self.current_action.left.action_func is not None:
        #     ret &= self.CheckArmFinished(self.current_action.left)

        # if self.current_action.right.action_func is not None:
        #     ret &= self.CheckArmFinished(self.current_action.right)
        
        # self.is_executing = not ret
        left_done = self.CheckArmFinished(self.current_action.left) if self.current_action.left.action_func else True
        right_done = self.CheckArmFinished(self.current_action.right) if self.current_action.right.action_func else True
    ...
    def create_message(self, action_name, index, decision_logic=None, **kwargs):
        ...
        msg = Message()
        msg.index = index
        msg.action_func = self.action_map[action_name]
        msg.params = list(kwargs.values())  # ✅  params  list, `*params` 
        msg.decision_logic = decision_logic
        self.get_logger().info(f"🔍  Message: {action_name}, Decision Logic: {decision_logic}")
        return msg
    def add_action_to_sequence(self, action_name, index, decision_logic=None, **kwargs):
        ...
        if action_name not in self.action_map:
            self.get_logger().error(f": {action_name}")
            return
        msg = self.create_message(action_name, index, decision_logic, **kwargs)
        if msg is None:
            return

        #  (index=0) or  (index=1) 
        if index ==0:
            self.task_action_queue.add_message(msg, Message())
        else:
            self.task_action_queue.add_message(Message(), msg)
    ...
    def add_dual_arm_action(self, left_action_name, right_action_name, left_params, right_params, decision_logic=None):
        """
        
        :param left_action_name:  
        :param right_action_name: 
        :param left_params:       Parameters()
        :param right_params:      Parameters()
        :param decision_logic:    ()
        """
        left_msg = self.create_message(left_action_name, 0, decision_logic, **(left_params or {}))
        right_msg = self.create_message(right_action_name, 1, decision_logic, **(left_params or {}))

        if left_msg is None or right_msg is None:
            return

        # 
        self.task_action_queue.add_message(left_msg, right_msg)
        self.get_logger().info(f"Added dual-arm action: {left_action_name} & {right_action_name} to sequence.")
    ...
    def next_action(self):
        ...
        self.current_action =  self.task_action_queue.get_task()
        if self.current_action is None:
            self.is_executing = False
            self.get_logger().info(f" ")
            return
        ...
        self.get_logger().info(f" ")
        if self.current_action.left and self.current_action.left.action_func:
            self.current_action.left.action_func(self.current_action.left.index, *self.current_action.left.params)
            self.get_logger().info(f" {self.arm_controllers[self.current_action.left.index].name_prefix} : {self.current_action.left.action_func.__name__},Parameters: {self.current_action.left.params}")
        if self.current_action.right and self.current_action.right.action_func:
            self.current_action.right.action_func(self.current_action.right.index, *self.current_action.right.params)
            self.get_logger().info(f" {self.arm_controllers[self.current_action.right.index].name_prefix} : {self.current_action.right.action_func.__name__},Parameters: {self.current_action.right.params}")

        # self.current_action = self.action_queue.pop(0)
        # action_func, index, params, decision_logic = self.current_action

        self.is_executing = True
        self.action_history.append(self.current_action)  # e
    ...
    def test_dual_arm_controller(self):
        """"""
        if not self.arm_controllers[0].feedback_received or not self.arm_controllers[1].feedback_received :
            return None
        self.initialized = True
        self.get_logger().info("🔪 ")
        self.add_dual_arm_action(
            "moveTo", "moveTo",
            left_params={"position": [305.23+10, 4.00-10, -145.00, 3.1416, 0.0, 1.5808]},
            right_params={"position": [305.23-10, 4.00+10, -145.00, 3.1416, 0.0, 1.5808]}
        )

        #  closeGripper, openGripper
        self.add_dual_arm_action(
            "closeGripper", "openGripper",
            left_params={"force": 50},
            right_params={"force": 30}
)
    ...
    def onTimerCallback(self):
        ...
        if self.is_executing:
            ...
            return
        if self.task_action_queue.is_empty():
            self.is_executing = False
            self.start_next_task()  # ,
            return
        self.next_action()
    ...
    def next_action(self):
        ...
        self.get_logger().info(f" :{self.current_action}")
        if(self.current_action.left.action_func is not None):
            self.current_action.left.action_func(self.current_action.left.index, *self.current_action.left.params)
            self.get_logger().info(f" {self.arm_controllers[self.current_action.left.index].name_prefix} : {self.current_action.left.action_func.__name__},Parameters: {self.current_action.left.params}")
        if(self.current_action.right.action_func is not None):
    ...
    def start_next_task(self):
        ...
        if not self.task_queue:  # 
            return
        self.initialized = True
        if self.current_task is None or not self.task_action_queue.is_empty():  #  action_queue 
